        ...
        <span class="relative z-0 inline-flex shadow-sm rounded-md">
            ...
            @else
                <a href="{{ $paginator->previousPageUrl() }}"
                    class="relative inline-flex items-center px-3 py-2 text-sm font-medium bg-emerald-600 text-white rounded-l-md hover:bg-emerald-700">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd"
                            d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z"
                            clip-rule="evenodd" />
            ...
            @foreach ($elements as $element)
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        ...
                        @else
                            <a href="{{ $url }}"
                                class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium bg-emerald-600 text-white hover:bg-emerald-700">{{ $page }}</a>
                        @endif
                    @endforeach
                @endif
            @endforeach
            ...
            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}"
                    class="relative inline-flex items-center px-3 py-2 -ml-px text-sm font-medium bg-emerald-600 text-white rounded-r-md hover:bg-emerald-700">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd"
